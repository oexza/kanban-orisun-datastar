...
export const COMPRESSIBLE_CONTENT_TYPE_REGEX = /^\s*(?:text\/[^;\s]+|application\/(?:javascript|json|xml|xml-dtd|ecmascript|dart|postscript|rtf|tar|toml|vnd\.dart|vnd\.ms-fontobject|vnd\.ms-opentype|wasm|x-httpd-php|x-javascript|x-ns-proxy-autoconfig|x-sh|x-tar|x-virtualbox-hdd|x-virtualbox-ova|x-virtualbox-ovf|x-virtualbox-vbox|x-virtualbox-vdi|x-virtualbox-vhd|x-virtualbox-vmdk|x-www-form-urlencoded)|font\/(?:otf|ttf)|image\/(?:bmp|vnd\.adobe\.photoshop|vnd\.microsoft\.icon|vnd\.ms-dds|x-icon|x-ms-bmp)|message\/rfc822|model\/gltf-binary|x-shader\/x-fragment|x-shader\/x-vertex|[^;\s]+?\+(?:json|text|xml|yaml))(?:[;\s]|$)/i;
export const shouldCompress = (res) => {
 ...
 */
export function getCompressionStream(encoding) {
    switch (encoding) {
        case "br":
 ...
 */
function createBrotliCompressTransformStream() {
    ...
    return new TransformStream({
        ...
        async transform(chunk, controller) {
            ...
            if (!canWrite && !isCancelled) {
                ...
                await new Promise((resolve) => brotliCompress.once("drain", resolve));
            }
        },
        flush(controller) {
            if (!brotliCompress || isCancelled) {
 ...
 * @returns A TransformStream<Uint8Array, Uint8Array>
 */
function createGzipCompressTransformStream() {
    let gzipCompress = null;
    let isCancelled = false;
    return new TransformStream({
        start(controller) {
        ...
        async transform(chunk, controller) {
            ...
            if (!canWrite && !isCancelled) {
                // Handle backpressure from the Node.js stream
                await new Promise((resolve) => gzipCompress.once("drain", resolve));
            }
        },
        flush(controller) {
            if (!gzipCompress || isCancelled) {
...
export const compression = createMiddleware(async (ctx, next) => {
    ...
    if (alreadyEncoded || // already encoded
    ...
    const ENCODING_TYPES = ["br", "gzip"];
    const accepted = ctx.req.header("Accept-Encoding");
    const encoding = ENCODING_TYPES.find((encoding) => accepted?.includes(encoding));
