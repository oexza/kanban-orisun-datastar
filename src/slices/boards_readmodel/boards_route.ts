...
} from "~/slices/boards_readmodel/boards_projection";
import {sendKeepAlivePing, ssePatch} from "~/lib/datastar";

const logger = createModuleLogger("boards-readmodel-route");

export const createBoardsRoute = (honoApp: HonoType, db: NodePgDatabase, subscriber: Subscriber) => {
    honoApp.get("/", async (c) => {
...
export const createBoardsSSERoute = (honoApp: HonoType, db: NodePgDatabase, subscriber: Subscriber) => {
...
function getSession(c: Context<HonoContextType>) {
    const sessionx = c.get("session")
    if (!sessionx.get("session_id")) {
        sessionx.set("session_id", randomUUIDv7().replaceAll("-", ""))
    }
...
async function getBoards(db: NodePgDatabase): Promise<BoardSummary[]> {
    return db
        .select({
            id: boards.id,
            title: boards.title,
