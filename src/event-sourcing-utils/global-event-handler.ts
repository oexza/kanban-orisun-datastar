...
import {EventHandlerCheckpointer, EventsSubscriber, Query, ResolvedDomainEvent} from "./types";

const logger = createModuleLogger('global-event-handler');

export class GlobalEventHandler {
    private isSubscribing = false;
    private readonly baseEventRetryDelay: number = 1000; // 1 second
    private readonly baseCheckpointRetryDelay: number = 500; // 0.5 seconds
    ...
    constructor(
        ...
        private readonly maxEventRetries: number = -1,
        private readonly handleEvent: (event: ResolvedDomainEvent<any>) => Promise<void>,
    ...
    private async retryCheckpointUpdate(eventHandlerName: string, position: any, retryCount: number = 0): Promise<void> {
        try {
    ...
    private async retryEventProcessing(event: ResolvedDomainEvent<any>, retryCount: number = 0): Promise<void> {
        try {
            // Process the event
    ...
    private async processEvent(event: ResolvedDomainEvent<any>): Promise<void> {
    ...
    public async startSubscribing() {
        ...
        const subscribeWithRetry = async (retryCount = 0) => {
            ...
            try {
                ...
                await this.eventsSubscriber.subscribeToEvents(
                    this.name,
                    async (event) => {
                        await this.processEvent(event as ResolvedDomainEvent<any>);
                    },
                    this.query,
                    await lastPosition(),
                    (error) => {
