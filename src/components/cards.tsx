...
export function CardList({
    ...
    const dropHandler = `evt.preventDefault();
     ...
     const cardsInList = evt.currentTarget.querySelectorAll('[role="listitem"]');
     ...
     for (let i = 0; i < cardsInList.length; i++) {
      ...
      const cardMiddle = rect.top + rect.height / 2;
       if (dropY < cardMiddle) { insertPosition = i; break; } 
     } 
     const allCardIds = Array.from(cardsInList).filter(card => card.dataset.cardId !== cardId).map(card => card.dataset.cardId); 
     allCardIds.splice(insertPosition, 0, cardId); 
     $cardIds = allCardIds; @put('/board/` + boardId + `/card/' + cardId + '/position')`;
